# MachineLearningLesBGS

## TODO:
* remplir le readme
* commenter tout le code
* changer les 6 functions models (celle voulue par le project description doivent retourner (w,loss))
* run les 6 models et avoir les erreures et les ajouter dans le rapport comme justification
* run avec et sans le 4 jets, ajouter les résultat au rapport
* run avec et sans les features processing, ajouter les résults au rapports
* faire une image de correlation pour expliquer la feature distance (Nico)
* finir cross-validation (Nico)
* run cross-validation pour trouver lambda/gamma cool (Nico)
* faire graphique des lambda/gamma cool et les ajouter au rapport (Nico)
* ranger le dossier du projet, (un dossier visualisation pour les histogrammes, etc)
* mettre le code que l'on veut dans le run.py
* préparer les fichier que l'on envoye dans un zip (on envoye pas les csv ou le milliard d'image?)
* finir le rapport

