...
split = int(np.ceil(0.1*len(index)))
np.random.shuffle(index)

new_y_train = y_train[index[split:]]
new_x_train = data_train[index[split:]]

w_rid, loss_train_rid = ridge_regression(new_y_train, new_x_train, 0.05)
...
"""w_lss, loss_train_lss = least_squares_SGD(y_train, data_train)
